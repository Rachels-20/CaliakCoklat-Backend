package com.pbl.kelembapantanah.service;

import java.time.DayOfWeek;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.util.ArrayList;
import java.util.List;
import java.util.OptionalDouble;

import org.springframework.amqp.rabbit.core.RabbitTemplate;
import org.springframework.stereotype.Service;
import org.springframework.web.client.RestTemplate;

import com.pbl.kelembapantanah.config.RabbitMQConfig;
import com.pbl.kelembapantanah.dto.DeviceResponse;
import com.pbl.kelembapantanah.dto.KelembapanTanahRequest;
import com.pbl.kelembapantanah.dto.KelembapanTanahResponse;
import com.pbl.kelembapantanah.dto.SensorDataEvent;
import com.pbl.kelembapantanah.dto.SensorStatusEvent;
import com.pbl.kelembapantanah.entity.KelembapanTanah;
import com.pbl.kelembapantanah.repository.KelembapanTanahRepository;

import lombok.RequiredArgsConstructor;

@Service
@RequiredArgsConstructor
public class KelembapanTanahService {

        private final KelembapanTanahRepository repository;
        private final RestTemplate restTemplate;
        private final RabbitTemplate rabbitTemplate;
        private static final String DEVICE_URL = "http://device-service:8092/devices/kode/";

        /**
         * Simpan data kelembapan tanah.
         */
        public KelembapanTanahResponse saveData(
                        KelembapanTanahRequest request) {

                DeviceResponse device = restTemplate.getForObject(
                                DEVICE_URL + request.getKodePerangkat(),
                                DeviceResponse.class);

                if (device == null) {
                        throw new RuntimeException("Device tidak ditemukan");
                }

                // Ambil data terakhir
                KelembapanTanah lastData = repository.findTopByDeviceIdOrderByWaktuDesc(device.getId());

                // Cek apakah status sensor berubah
                boolean statusChanged = lastData != null &&
                                lastData.isSensorOK() != request.isSensorOK();

                System.out.println(
                                "[DATA] " +
                                                request.getKodePerangkat() +
                                                " -> deviceId=" +
                                                device.getId());

                // Simpan data
                KelembapanTanah data = new KelembapanTanah();

                data.setDeviceId(device.getId());
                data.setNilai(request.getNilai());
                data.setSensorOK(request.isSensorOK());
                data.setWaktu(LocalDateTime.now());

                KelembapanTanah saved = repository.save(data);

                // ==========================
                // Event data sensor (selalu)
                // ==========================
                SensorDataEvent dataEvent = new SensorDataEvent();

                dataEvent.setDeviceId(saved.getDeviceId());
                dataEvent.setSensorType("KELEMBAPAN_TANAH");
                dataEvent.setValue(saved.getNilai());

                rabbitTemplate.convertAndSend(
                                RabbitMQConfig.EXCHANGE,
                                RabbitMQConfig.ROUTING_KEY,
                                dataEvent);

                // ==========================
                // Event status sensor
                // (hanya jika berubah)
                // ==========================
                if (statusChanged) {

                        SensorStatusEvent statusEvent = new SensorStatusEvent();

                        statusEvent.setDeviceId(saved.getDeviceId());
                        statusEvent.setSensorType("KELEMBAPAN_TANAH");
                        statusEvent.setSensorOK(saved.isSensorOK());

                        rabbitTemplate.convertAndSend(
                                        RabbitMQConfig.EXCHANGE,
                                        RabbitMQConfig.STATUS_ROUTING_KEY,
                                        statusEvent);
                }

                return KelembapanTanahResponse.builder()
                                .id(saved.getId())
                                .deviceId(saved.getDeviceId())
                                .nilai(saved.getNilai())
                                .waktu(saved.getWaktu())
                                .build();
        }

        /**
         * Ambil data terbaru berdasarkan deviceId.
         */
        public KelembapanTanahResponse getLatest(Long deviceId) {

                KelembapanTanah latest = repository.findTopByDeviceIdOrderByWaktuDesc(deviceId);

                if (latest == null) {
                        throw new RuntimeException("Data tidak ditemukan");
                }

                return KelembapanTanahResponse.builder()
                                .id(latest.getId())
                                .deviceId(latest.getDeviceId())
                                .nilai(latest.getNilai())

                                .waktu(latest.getWaktu())
                                .build();
        }

        /**
         * Ambil rata-rata data mingguan.
         */
        public List<Double> getWeekly(Long deviceId, String date) {

                LocalDate inputDate = LocalDate.parse(date);

                LocalDate start = inputDate.with(DayOfWeek.MONDAY);
                LocalDate end = inputDate.with(DayOfWeek.SUNDAY);

                List<Double> result = new ArrayList<>();

                for (LocalDate day = start; !day.isAfter(end); day = day.plusDays(1)) {

                        List<KelembapanTanah> dailyData = repository.findByDeviceIdAndWaktuBetween(
                                        deviceId,
                                        day.atStartOfDay(),
                                        day.atTime(23, 59, 59));

                        OptionalDouble optionalAverage = dailyData.stream()
                                        .mapToDouble(KelembapanTanah::getNilai)
                                        .average();

                        Double average = optionalAverage.isPresent()
                                        ? optionalAverage.getAsDouble()
                                        : null;

                        result.add(average);
                }

                return result;
        }

}